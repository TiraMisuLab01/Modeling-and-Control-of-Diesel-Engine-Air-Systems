...
% --- Design 2: Aggressive Control (Successful Design) ---
fprintf('\n--- LQR Design 2: Aggressive (Successful) ---\n');
Q2 = 100 * (C' * C); % Penalize state error more
R2 = 0.5 * eye(2);  % Allow more control effort
K2 = lqr(A, B, Q2, R2);
sys_cl2 = ss(A - B * K2, B, C, 0);
fprintf('K2 (Q=100*C''C, R=0.5*I):\n');
disp(K2);


%% Step 3: Simulation and Performance Analysis

%% [FIXED] 修复 Figure 1 的布局和标签问题
% --- Figure 1: Step Response for Design 1 (Baseline) ---
figure('Position', [100, 100, 700, 500]); % [left, bottom, width, height]
% 1. 让 step() 自动创建 2x2 布局
step(sys_cl1, 10); 
% 2. 获取 step() 创建的所有 axes 句柄
all_axes = findobj(gcf, 'type', 'axes');
all_axes = flipud(all_axes); % 翻转顺序，使其符合 1,2,3,4
% 3. 循环遍历所有 axes 并设置标签
for i = 1:length(all_axes)
    ax = all_axes(i);
    set(ax, 'YTickMode', 'auto', 'YLimMode', 'auto'); % 强制显示 Y 轴刻度
    xlabel(ax, 'Time (s)');
    ylabel(ax, 'Amplitude');
    grid(ax, 'on');
end
% 4. 添加一个干净的顶部标题
sgtitle('Step Response - Design 1 (Baseline)', 'FontSize', 12);
info1 = stepinfo(sys_cl1);

%% [FIXED] 修复 Figure 2 的布局和标签问题
% --- Figure 2: Step Response for Design 2 (Aggressive) ---
figure('Position', [150, 150, 700, 500]);
% 1. 让 step() 自动创建 2x2 布局
step(sys_cl2, 10); 
% 2. 获取 step() 创建的所有 axes 句柄
all_axes = findobj(gcf, 'type', 'axes');
all_axes = flipud(all_axes); % 翻转顺序
% 3. 循环遍历所有 axes 并设置标签
for i = 1:length(all_axes)
    ax = all_axes(i);
    set(ax, 'YTickMode', 'auto', 'YLimMode', 'auto'); % 强制显示 Y 轴刻度
...
% 4. 添加一个干净的顶部标题
sgtitle('Step Response - Design 2 (Aggressive / Successful)', 'FontSize', 12);
...
% --- Figure 3 (原 Figure 2) 布局为 2x2 ---
figure('Position', [200, 100, 900, 600]); % 调整窗口大小

% --- Design 1 Row ---
subplot(2,2,1); 
plot(t_sim, x_ic1); 
title('State Response - Design 1 (Baseline)'); 
...
subplot(2,2,2); 
plot(t_sim, u_ic1); 
title('Control Signal - Design 1 (Baseline)'); 
...
subplot(2,2,3); 
plot(t_sim, x_ic2); 
title('State Response - Design 2 (Aggressive)'); 
legend('x1','x2','x3','x4', 'Location', 'northeast'); 
grid on;
xlabel('Time (s)');
ylabel('States (x)');
...
subplot(2,2,4); 
plot(t_sim, u_ic2); 
title('Control Signal - Design 2 (Aggressive)'); 
legend('u1','u2', 'Location', 'northeast'); 
grid on;
xlabel('Time (s)');
ylabel('Control Input (u)');
